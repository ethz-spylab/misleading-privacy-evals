#!/bin/bash
#SBATCH --array=0-3199
#SBATCH --job-name=selena-aux-splitai
#SBATCH --gpus=rtx_3090:1
#SBATCH --mem-per-cpu=2G
#SBATCH --ntasks=1 --cpus-per-task=8
#SBATCH --time=01:00:00
# Takes ~25min per teacher, but request 1h to be safe
# Train 64 shadow models x 25 teachers x 2 settings = 3200 models
...
TEACHER_IDX=$((TASK_IDX % NUM_TEACHER))
TASK_IDX=$((TASK_IDX / NUM_TEACHER))
...
echo "Running task ${SLURM_ARRAY_TASK_ID} experiment ${EXPERIMENT}, shadow model ${SHADOW_MODEL_IDX}, teacher ${TEACHER_IDX}"
...
python -u -m experiments.selena --experiment-dir "${EXPERIMENT_DIR}" --experiment "${EXPERIMENT}" --data-dir "${DATA_DIR}" \
 ...
 splitai --teacher-idx "${TEACHER_IDX}"
