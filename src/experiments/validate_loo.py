...
def _run_attack(
    ...
    if not fixed_halves:
        score_maps["full dataset"] = (shadow_scores_full, shadow_membership_mask, "_full")

    # Global threshold
    print("# Global threshold")
    for score_map_name, (score_map, membership_mask, score_map_suffix) in score_maps.items():
        print(f"## on {score_map_name}")
