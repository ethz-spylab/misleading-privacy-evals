...
def main():
    ...
    if args.action == "attack":
        ...
        full_data, membership_mask, canary_mask, poison_mask = data_generator.build_train_data_full_with_poison(
            shadow_model_idx
        )
...
def _run_train_splitai(
    ...
    print(f"Training teacher {teacher_idx} for shadow model {shadow_model_idx}")
    print(
    ...
    output_dir = directory_manager.get_training_teacher_output_dir(shadow_model_idx, teacher_idx)
    ...
    with run:
        ...
        train_pred_full = _predict(current_model, train_data_full_pipe, data_augmentation=False)
        ...
        train_membership_mask, canary_mask = splitai_handler.get_teacher_masks(teacher_idx)
...
def _run_train_distillation(
    ...
    print(f"Distilling student for shadow model {shadow_model_idx}")
    print(
        f"{data_generator.num_canaries} canaries ({data_generator.canary_type.value}), "
        f"{data_generator.num_poison} poisons ({data_generator.poison_type.value})"
    )
    ...
    print("Loading raw teacher predictions")
    ...
    for teacher_idx in range(splitai_handler.num_teachers):
        ...
        teacher_dir = directory_manager.get_training_teacher_output_dir(shadow_model_idx, teacher_idx)
        teacher_predictions_test.append(torch.load(teacher_dir / "predictions_test.pt")[:, 0])
        teacher_predictions_train.append(torch.load(teacher_dir / "predictions_train.pt")[:, 0])
    ...
    with run:
        ...
        print("Performing Split-AI inference")
        metrics.update(splitai_handler.evaluate_ensemble(teacher_predictions_test, test_ys))
        metrics.update(splitai_handler.evaluate_splitai_test(teacher_predictions_test, test_ys))
        del teacher_predictions_test
        splitai_unreduced_logits = splitai_handler.infer_splitai_train_full(teacher_predictions_train)
        torch.save(splitai_unreduced_logits, output_dir / "splitai_predictions_train_raw.pt")
        splitai_probs_train = splitai_handler.softlabels_from_raw(splitai_unreduced_logits)
        ...
        # Evaluate Split-AI
        print("Evaluating Split-AI")
        print(f"Split-AI test accuracy: {metrics['splitai_test_accuracy']:.4f}")

        splitai_train_ys_pred = torch.argmax(splitai_probs_train, dim=-1)
        splitai_correct_train = torch.eq(splitai_train_ys_pred, splitai_train_ys).to(dtype=base.DTYPE_EVAL)
        metrics.update(
            {
                "splitai_train_accuracy_full": torch.mean(splitai_correct_train).item(),
                "splitai_train_accuracy_in": torch.mean(splitai_correct_train[splitai_membership_mask]).item(),
                "splitai_train_accuracy_out": torch.mean(splitai_correct_train[~splitai_membership_mask]).item(),
            }
        )
        print(f"Split-AI train accuracy (full data w/ duplicates): {metrics['splitai_train_accuracy_full']:.4f}")
        print(f"Split-AI train accuracy (only IN samples w/ duplicates): {metrics['splitai_train_accuracy_in']:.4f}")
        print(f"Split-AI train accuracy (only OUT samples): {metrics['splitai_train_accuracy_out']:.4f}")
        metrics.update(
            {
                "splitai_train_accuracy_canaries": torch.mean(splitai_correct_train[splitai_canary_mask]).item(),
        ...
        print(f"Split-AI train accuracy (full canary subset): {metrics['splitai_train_accuracy_canaries']:.4f}")
        print(f"Split-AI train accuracy (IN canary subset): {metrics['splitai_train_accuracy_canaries_in']:.4f}")
        print(f"Split-AI train accuracy (OUT canary subset): {metrics['splitai_train_accuracy_canaries_out']:.4f}")
...
def _run_attack(
    ...
    for shadow_model_idx in range(data_generator.num_shadow):
        shadow_model_dir = directory_manager.get_training_student_output_dir(shadow_model_idx)
        shadow_splitai_logits_raw.append(torch.load(shadow_model_dir / "splitai_predictions_train_raw.pt"))
    shadow_splitai_logits = torch.stack(shadow_splitai_logits_raw, dim=1).to(dtype=base.DTYPE_EVAL)
    ...
    splitai_scores_full = {score_name: scores.unsqueeze(2) for score_name, scores in splitai_scores_full.items()}

    # Attack Split-AI, query on canaries
    print("# Split-AI, query on canaries")
    _run_individual_attack(
        {score_name: scores[canary_mask_sai] for score_name, scores in splitai_scores_full.items()},
    ...
    if data_generator.num_poison > 0:
        ...
        print("# Split-AI, query on poison")
        _run_individual_attack(
            {score_name: scores[poison_mask_sai] for score_name, scores in splitai_scores_full.items()},
    ...
    if args.embeddings_file is not None and canary_type != data.CanaryType.DUPLICATES_MISLABEL_HALF:
        # Indices of non-canaries non-poison samples
        other_indices = torch.where((~canary_mask_sai) & (~poison_mask_sai))[0]
        assert other_indices.size() == (data_generator.num_raw_training_samples - data_generator.num_canaries,)
        ...
        canary_embeddings = canary_embeddings / canary_embeddings.norm(dim=1, keepdim=True)
        ...
        canary_nn_indices, other_nn_indices = scipy.optimize.linear_sum_assignment(similarities, maximize=True)
        ...
        splitai_scores = {
            ...
            "logit": attack_util.logit_score_from_probs(
                SplitAIHandler.softlabels_from_raw(shadow_splitai_logits[nearest_neighbor_indices]),
                attack_ys_sai[canary_indices],
            ).unsqueeze(2),
        }

        print("# Split-AI, query on near-duplicates")
        _run_individual_attack(
            splitai_scores,
            shadow_membership_mask_sai[canary_indices],
            output_dir,
    ...
    student_scores = {
        "hinge": attack_util.hinge_score(shadow_logits[canary_indices], attack_ys[canary_indices]),
        "logit": attack_util.logit_score(shadow_logits[canary_indices], attack_ys[canary_indices]),
    }

    if canary_type != data.CanaryType.DUPLICATES_MISLABEL_HALF:
        assert all(
            scores.size() == (data_generator.num_canaries, data_generator.num_shadow, num_augmentations)
            for scores in student_scores.values()
        )
    else:
        assert all(
            scores.size() == (data_generator.num_canaries // 2, data_generator.num_shadow, num_augmentations)
            for scores in student_scores.values()
        )
...
def _run_individual_attack(
    ...
    assert all(scores.size(0) == shadow_membership_mask.size(0) for scores in shadow_scores.values())  # num samples
    assert all(scores.size(1) == shadow_membership_mask.size(1) for scores in shadow_scores.values())  # num shadow
...
class SplitAIHandler(object):
    def __init__(
        ...
        self._query_indices_in = torch.empty((num_in_samples, self._num_query), dtype=torch.long)
        ...
        self._query_indices_full = torch.empty((len(self._full_data), self._num_query), dtype=torch.long)
        ...
        for sample_idx in range(len(self._full_data)):
            if not self._membership_mask[sample_idx]:
                self._query_indices_full[sample_idx] = self._query_indices_in[rng_query_out.integers(0, num_in_samples)]
        del rng_query_out

        # Store rng for inference on test data to be reproducible
        rng_test_inference, rng = rng.spawn(2)
        self._test_inference_seed = rng_test_inference.integers(0, 2**32, dtype=np.uint32)
    ...
    def get_teacher_masks(self, teacher_idx: int) -> typing.Tuple[torch.Tensor, torch.Tensor]:
    ...
    def evaluate_ensemble(self, teacher_predictions: torch.Tensor, targets: torch.Tensor) -> typing.Dict[str, float]:
        ensemble_probabilities = torch.mean(torch.softmax(teacher_predictions, dim=-1), dim=1)
        correct_predictions = torch.eq(torch.argmax(ensemble_probabilities, dim=-1), targets).to(dtype=base.DTYPE_EVAL)
        return {
    ...
    def evaluate_splitai_test(
        ...
        query_indices = torch.from_numpy(rng.choice(self._query_indices_in, size=len(targets), replace=True))
        splitai_probabilities = self._splitai_inference(teacher_predictions, query_indices)
        correct_predictions = torch.eq(torch.argmax(splitai_probabilities, dim=-1), targets).to(dtype=base.DTYPE_EVAL)
        return {
    ...
    def build_distillation_dataset(self, splitai_train_probs_full: torch.Tensor) -> data.Dataset:
    ...
    def _splitai_inference(
        self, teacher_predictions: torch.Tensor, query_indices: torch.Tensor, return_unreduced_logits: bool = False
    ):
        # Get raw log probs
        query_indices = query_indices.unsqueeze(-1).tile((1, 1, teacher_predictions.size(-1)))
    ...
    @classmethod
    def softlabels_from_raw(cls, splitai_unreduced_log_probs: torch.Tensor) -> torch.Tensor:
        assert splitai_unreduced_log_probs.dim() >= 3
        return torch.mean(torch.softmax(splitai_unreduced_log_probs, dim=-1), dim=-2)

    def get_splitai_eval_data(self) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
...
class DirectoryManager(object):
    ...
    def get_training_teacher_output_dir(self, shadow_model_idx: int, teacher_idx: int) -> pathlib.Path:
        return self._get_shadow_model_base_dir(shadow_model_idx) / "teacher" / str(teacher_idx)
    ...
    def get_training_log_dir(self, shadow_model_idx: int, teacher_idx: typing.Optional[int]) -> pathlib.Path:
...
def parse_args() -> argparse.Namespace:
    ...
    # Defense-specific
    parser.add_argument("--num-teachers", type=int, required=True, help="Number of ensemble members (K)")
    parser.add_argument("--num-query", type=int, required=True, help="Number of queries per sample (L)")
    ...
    train_parser.add_argument("--shadow-model-idx", type=int, required=True, help="Train shadow model with index")

    # Defense-specific
    train_subparser = train_parser.add_subparsers(dest="train_action", required=True, help="Mechanism to train")
    # Train: Split-AI
    train_splitai_parser = train_subparser.add_parser("splitai")
    train_splitai_parser.add_argument("--teacher-idx", type=int, required=True, help="Index of teacher to train")
    # Train: Distillation
    ...
    attack_parser.add_argument(
        "--embeddings-file", type=pathlib.Path, required=False, help="Embeddings to determine nearest neighbors"
    )
