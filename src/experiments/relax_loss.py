...
def _run_train(
    ...
    print(f"Training shadow model {shadow_model_idx}, target loss {target_loss}, data augmentation {data_augmentation}")
    print(
        ...
        for batch_xs, batch_ys in tqdm.tqdm(
            ...
            if epoch % 2 == 0:
                ...
                if batch_loss_ce > target_loss:
                    ...
                    batch_conf_targets = batch_pred_probs[torch.arange(batch_ys.size(0)), batch_ys]
                    batch_conf_others = (1.0 - batch_conf_targets) / (num_classes - 1)
                    batch_soft_labels = batch_conf_others.unsqueeze(-1) * torch.ones(
                        (batch_ys.size(0), num_classes), dtype=base.DTYPE, device=base.DEVICE
                    )
                    batch_soft_labels[torch.arange(batch_ys.size(0)), batch_ys] = batch_conf_targets
                    batch_soft_labels = batch_soft_labels.detach()  # NB: original code has no detach!!

                    batch_loss_soft_full = loss(input=batch_pred, target=batch_soft_labels)
                    ...
                    batch_loss = torch.mean((1.0 - batch_pred_correct) * batch_loss_soft_full - batch_loss_ce_full)
...
def parse_args() -> argparse.Namespace:
    ...
    # Defense-specific
    train_parser.add_argument("--target-loss", type=float, default=1.0, help="Target loss (alpha)")
    train_parser.add_argument(
        "--data-augmentation", action="store_true", help="Use data augmentation during training and attack"
    )
